// TODO: Why does this only work in root of project?
 ...
 */
function setupScene() {
    // TODO: Why does this not have an input?
    ...
    if (cameraSource) {
        ...
        const cameraScaleFactor =
            1.0 / ((3.0 * cameraSource.width) / outputWidth);
        cameraItem.scale = { x: cameraScaleFactor, y: cameraScaleFactor };
        cameraItem.position = {
            x:
                outputWidth -
                cameraSource.width * cameraScaleFactor -
                outputWidth / 10,
            y:
                outputHeight -
...
/**
 * Is Virtual Cam Plugin Installed
 *
 * @returns Boolean
 */
...
function getCameraSource() {
    ...
    for (let i = 1; i <= 4; i++) {
        if (obsCameraInput.width === 0) {
            const waitMs = 100 * i;
            console.debug(
                `Waiting for ${waitMs}ms until camera get initialized.`
            );
